...
n_states_to_return = 3

for i, ratio in enumerate(ratios): # i is the graphic index
    # normalization factor E01 at ng = 0.5
    energies_ss = get_transmon_energies(n_states_to_return, 0.5, ratio) # energy 0 at sweet spot
    ...
    e0_min = np.min(all_energies[:, 0]) # minimum of ground state energy, take only first column from all rows for E0
    # will set the zero point energy as the bottom of m = 0 level

    for m in levels:
        energies_raw = all_energies[:, m]
        # Normalization: (E - min(E0)) / E01_ref
        energies_norm = (energies_raw - e0_min) / E01_ref
        axes[i].plot(ng_axis, energies_norm, color=colors[m], label=f'm={m}')
        # i is the graphic index

    # Formatare grafic conform originalului
    axes[i].set_title(f'EJ/EC = {ratio}')
    axes[i].set_xlabel('$n_g$')
    axes[i].set_ylabel('$E_m / E_{01}$')
    axes[i].set_ylim(-0.5, 10 if ratio == 1.0 else 3)  # adjusting Y scale
